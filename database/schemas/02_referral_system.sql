-- OPEN CLAW: 紹介制度関連テーブル
-- 紹介ツリーと報酬計算を管理

-- 紹介ツリーテーブル
CREATE TABLE referral_tree (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    member_id TEXT NOT NULL REFERENCES members(member_id) ON DELETE CASCADE,
    ref_level1_id TEXT REFERENCES members(member_id) ON DELETE SET NULL,
    ref_level2_id TEXT REFERENCES members(member_id) ON DELETE SET NULL,
    ref_level3_id TEXT REFERENCES members(member_id) ON DELETE SET NULL,
    referral_code TEXT NOT NULL,
    registered_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    
    -- 制約: 自己紹介防止
    CONSTRAINT no_self_referral CHECK (
        member_id != ref_level1_id AND
        member_id != ref_level2_id AND
        member_id != ref_level3_id
    )
);

-- インデックス作成
CREATE INDEX idx_referral_tree_member_id ON referral_tree(member_id);
CREATE INDEX idx_referral_tree_level1 ON referral_tree(ref_level1_id);
CREATE INDEX idx_referral_tree_level2 ON referral_tree(ref_level2_id);
CREATE INDEX idx_referral_tree_level3 ON referral_tree(ref_level3_id);
CREATE INDEX idx_referral_tree_code ON referral_tree(referral_code);

-- 報酬ログテーブル
CREATE TABLE reward_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    member_id TEXT NOT NULL REFERENCES members(member_id) ON DELETE CASCADE,
    referred_member TEXT NOT NULL REFERENCES members(member_id) ON DELETE CASCADE,
    level INTEGER NOT NULL CHECK (level IN (1, 2, 3)),
    amount NUMERIC NOT NULL CHECK (amount >= 0),
    status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'paid', 'failed')),
    payment_month TEXT NOT NULL, -- 'YYYY-MM' format
    paid_at TIMESTAMP WITH TIME ZONE,
    tx_hash TEXT, -- MINARA送金トランザクションID
    failure_reason TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- インデックス作成
CREATE INDEX idx_reward_logs_member_id ON reward_logs(member_id);
CREATE INDEX idx_reward_logs_referred_member ON reward_logs(referred_member);
CREATE INDEX idx_reward_logs_status ON reward_logs(status);
CREATE INDEX idx_reward_logs_payment_month ON reward_logs(payment_month);
CREATE INDEX idx_reward_logs_level ON reward_logs(level);

-- 更新時刻自動更新
CREATE TRIGGER update_reward_logs_updated_at BEFORE UPDATE
    ON reward_logs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Row Level Security
ALTER TABLE referral_tree ENABLE ROW LEVEL SECURITY;
ALTER TABLE reward_logs ENABLE ROW LEVEL SECURITY;

-- 自分に関連する紹介ツリーのみ閲覧可能
CREATE POLICY "Users can view own referral data" ON referral_tree
    FOR SELECT USING (
        member_id = auth.uid()::text OR
        ref_level1_id = auth.uid()::text OR
        ref_level2_id = auth.uid()::text OR
        ref_level3_id = auth.uid()::text
    );

-- 自分の報酬ログのみ閲覧可能
CREATE POLICY "Users can view own rewards" ON reward_logs
    FOR SELECT USING (member_id = auth.uid()::text);

-- 管理者フルアクセス
CREATE POLICY "Master admin full access referral" ON referral_tree
    FOR ALL USING (auth.uid()::text = 'master_001');

CREATE POLICY "Master admin full access rewards" ON reward_logs
    FOR ALL USING (auth.uid()::text = 'master_001');

-- 紹介ツリー構築関数
CREATE OR REPLACE FUNCTION build_referral_tree(
    new_member_id TEXT,
    used_referral_code TEXT
)
RETURNS VOID AS $$
DECLARE
    level1_member TEXT;
    level2_member TEXT;
    level3_member TEXT;
    current_payment_month TEXT;
BEGIN
    -- 現在の月を取得 (YYYY-MM)
    current_payment_month := TO_CHAR(NOW(), 'YYYY-MM');
    
    -- 紹介コードの持ち主を特定（1段目）
    SELECT member_id INTO level1_member
    FROM members 
    WHERE referral_code = used_referral_code 
    AND membership_status = 'active';
    
    IF level1_member IS NOT NULL THEN
        -- 1段目の紹介者を特定できた場合
        
        -- 2段目の紹介者を特定
        SELECT ref_level1_id INTO level2_member
        FROM referral_tree
        WHERE member_id = level1_member;
        
        -- 3段目の紹介者を特定
        IF level2_member IS NOT NULL THEN
            SELECT ref_level1_id INTO level3_member
            FROM referral_tree
            WHERE member_id = level2_member;
        END IF;
        
        -- 紹介ツリーに登録
        INSERT INTO referral_tree (
            member_id,
            ref_level1_id,
            ref_level2_id,
            ref_level3_id,
            referral_code
        ) VALUES (
            new_member_id,
            level1_member,
            level2_member,
            level3_member,
            used_referral_code
        );
        
        -- 報酬計上
        -- 1段目報酬
        INSERT INTO reward_logs (
            member_id,
            referred_member,
            level,
            amount,
            payment_month
        ) VALUES (
            level1_member,
            new_member_id,
            1,
            200,
            current_payment_month
        );
        
        -- 2段目報酬
        IF level2_member IS NOT NULL THEN
            INSERT INTO reward_logs (
                member_id,
                referred_member,
                level,
                amount,
                payment_month
            ) VALUES (
                level2_member,
                new_member_id,
                2,
                50,
                current_payment_month
            );
        END IF;
        
        -- 3段目報酬
        IF level3_member IS NOT NULL THEN
            INSERT INTO reward_logs (
                member_id,
                referred_member,
                level,
                amount,
                payment_month
            ) VALUES (
                level3_member,
                new_member_id,
                3,
                50,
                current_payment_month
            );
        END IF;
        
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 月次報酬サマリービュー
CREATE VIEW monthly_reward_summary AS
SELECT 
    member_id,
    payment_month,
    COUNT(*) as referral_count,
    SUM(amount) as total_amount,
    SUM(CASE WHEN status = 'paid' THEN amount ELSE 0 END) as paid_amount,
    SUM(CASE WHEN status = 'pending' THEN amount ELSE 0 END) as pending_amount,
    SUM(CASE WHEN level = 1 THEN amount ELSE 0 END) as level1_amount,
    SUM(CASE WHEN level = 2 THEN amount ELSE 0 END) as level2_amount,
    SUM(CASE WHEN level = 3 THEN amount ELSE 0 END) as level3_amount
FROM reward_logs
GROUP BY member_id, payment_month;

-- 紹介実績ビュー
CREATE VIEW referral_stats AS
SELECT 
    r.ref_level1_id as referrer_id,
    COUNT(DISTINCT r.member_id) as direct_referrals,
    COUNT(DISTINCT r2.member_id) as indirect_level2,
    COUNT(DISTINCT r3.member_id) as indirect_level3,
    COUNT(DISTINCT r.member_id) + 
    COUNT(DISTINCT r2.member_id) + 
    COUNT(DISTINCT r3.member_id) as total_network_size
FROM referral_tree r
LEFT JOIN referral_tree r2 ON r2.ref_level2_id = r.ref_level1_id
LEFT JOIN referral_tree r3 ON r3.ref_level3_id = r.ref_level1_id
WHERE r.ref_level1_id IS NOT NULL
GROUP BY r.ref_level1_id;