...
CREATE TABLE payment_logs (
    ...
    payment_type TEXT NOT NULL CHECK (
        payment_type IN ('initial', 'monthly', 'penalty', 'refund')
    ),
    ...
    tx_hash TEXT UNIQUE,
    payment_period TEXT, -- 月額の場合は 'YYYY-MM'
    memo TEXT,
    status TEXT DEFAULT 'confirmed' CHECK (
        status IN ('pending', 'confirmed', 'failed', 'disputed')
    ),
    ...
    -- インデックス付きカラム用制約
    CONSTRAINT valid_period_format CHECK (
        payment_period IS NULL OR 
        payment_period ~ '^\d{4}-\d{2}$'
    )
);

-- ゲートウェイメッセージテーブル（CLAW間通信）
CREATE TABLE gateway_messages (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    target TEXT NOT NULL, -- 'all' または member_id
...
CREATE INDEX idx_payment_logs_tx_hash ON payment_logs(tx_hash);
CREATE INDEX idx_payment_logs_payment_type ON payment_logs(payment_type);
CREATE INDEX idx_payment_logs_payment_period ON payment_logs(payment_period);
CREATE INDEX idx_payment_logs_confirmed_at ON payment_logs(confirmed_at);

CREATE INDEX idx_gateway_messages_target ON gateway_messages(target);
CREATE INDEX idx_gateway_messages_type ON gateway_messages(message_type);
CREATE INDEX idx_gateway_messages_created_at ON gateway_messages(created_at);
CREATE INDEX idx_gateway_messages_expires_at ON gateway_messages(expires_at);

CREATE INDEX idx_message_receipts_message_id ON message_receipts(message_id);
CREATE INDEX idx_message_receipts_member_id ON message_receipts(member_id);
CREATE INDEX idx_message_receipts_status ON message_receipts(status);

CREATE INDEX idx_trade_signals_signal_id ON trade_signals(signal_id);
...
-- トレードシグナルは全アクティブメンバーが閲覧可能
CREATE POLICY "Active members can view trade signals" ON trade_signals
    FOR SELECT USING (
        EXISTS (
...
-- 管理者フルアクセス
CREATE POLICY "Master admin full access payments" ON payment_logs
    FOR ALL USING (auth.uid()::text = 'master_001');

CREATE POLICY "Master admin full access messages" ON gateway_messages
    FOR ALL USING (auth.uid()::text = 'master_001');

CREATE POLICY "Master admin full access receipts" ON message_receipts
    FOR ALL USING (auth.uid()::text = 'master_001');

CREATE POLICY "Master admin full access signals" ON trade_signals
    FOR ALL USING (auth.uid()::text = 'master_001');
...
BEGIN
    ...
    IF NEW.payment_type = 'initial' AND NEW.status = 'confirmed' THEN
        ...
        SET 
            ...
            fee_paid_until = CURRENT_DATE + INTERVAL '1 month'
        WHERE member_id = NEW.member_id;
        
        -- 紹介ツリーを構築（referred_by_codeがある場合）
        IF EXISTS (
            SELECT 1 FROM members 
            WHERE member_id = NEW.member_id 
            AND referred_by_code IS NOT NULL
        ) THEN
            PERFORM build_referral_tree(
                NEW.member_id,
    ...
    ELSIF NEW.payment_type = 'monthly' AND NEW.status = 'confirmed' THEN
        ...
        SET 
            membership_status = 'active',
            fee_paid_until = CASE 
                WHEN fee_paid_until > CURRENT_DATE THEN fee_paid_until + INTERVAL '1 month'
...
GROUP BY TO_CHAR(confirmed_at, 'YYYY-MM'), payment_type
ORDER BY month DESC, payment_type;

-- アクティブメンバー統計ビュー
CREATE VIEW member_activity_stats AS
SELECT 
    membership_status,
